package io.mosip.resident.service.helper;
...
public class WebSubHelper {
	...
	/** The Constant logger. */
	private static final Logger logger = LoggerConfiguration.logConfig(WebSubHelper.class);
	...
	public int initSubscriber(WebSubEventSubcriber subscriber, Supplier<Boolean> enableTester) {
		try {
			...
			logger.error(ResidentConstants.SESSION_ID, "initSubscriber",  this.getClass().getSimpleName(), "FATAL: Subscription failed for:" + subscriber.getClass().getCanonicalName());
	...
	public int initRegistrar(WebSubEventTopicRegistrar registrar) {
		try {
			...
			logger.error(ResidentConstants.SESSION_ID, "initSubscriber",  this.getClass().getSimpleName(), "registration failed for:" + registrar.getClass().getCanonicalName());
			return HttpStatus.SC_SERVICE_UNAVAILABLE;
		} catch (Exception e) {
			//Just logging the exception to avoid other further subscriptions failure
			logger.error(ResidentConstants.SESSION_ID, "initSubscriber",  this.getClass().getSimpleName(), "Subscription failed for:" + registrar.getClass().getCanonicalName());
			return HttpStatus.SC_INTERNAL_SERVER_ERROR;
		}
	}
